...
public class Home extends javax.swing.JFrame {
   ...
   private void setButtonStyles() {
    ...
    private void loadClasses() {
        String query = "SELECT class_name, section, subject, room FROM classes";
        try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/crms", "root", "");
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(query)) {

            DefaultTableModel model = new DefaultTableModel(new String[]{"Class Name", "Section", "Subject", "Room"}, 0);
            while (rs.next()) {
                String className = rs.getString("class_name");
                String section = rs.getString("section");
                String subject = rs.getString("subject");
                String room = rs.getString("room");
                model.addRow(new Object[]{className, section, subject, room});
            }
