...
// Collect selected classes
        System.out.println("Collecting selected classes...");
        for (int i = 0; i < model.getRowCount(); i++) {
            Boolean isSelected = (Boolean) model.getValueAt(i, 0);
            Integer classId = (Integer) model.getValueAt(i, 1);
            String className = (String) model.getValueAt(i, 2);

            System.out.println("Row " + i + ": isSelected=" + isSelected + ", classId=" + classId + ", className=" + className);
        ...
        System.out.println("Classes selected for deletion: " + classIdsToDelete);
...
// Prepare for deletion and archiving
        Connection conn = null;
        PreparedStatement insertArchivePstmt = null;
        PreparedStatement deleteSchedulesPstmt = null;
        PreparedStatement deleteClassPstmt = null;
        ...
        try {
            ...
            // Prepare archive and delete statements
            String archiveQuery = "INSERT INTO archiveclasses (class_id, class_name, section_code, subject, time_deleted) "
                    + "SELECT class_id, class_name, section_code, subject, NOW() "
            ...
            insertArchivePstmt = conn.prepareStatement(archiveQuery);
            deleteSchedulesPstmt = conn.prepareStatement(deleteSchedulesQuery);
            deleteClassPstmt = conn.prepareStatement(deleteClassQuery);

            // Disable auto-commit for transaction support
            conn.setAutoCommit(false);

            // Archive and delete each selected class
            System.out.println("Archiving and deleting classes...");
            for (int i = 0; i < classIdsToDelete.size(); i++) {
                int classId = classIdsToDelete.get(i);
                String className = classNamesToDelete.get(i);

                System.out.println("Processing class: ID=" + classId + ", Name=" + className);

                try {
                    // Archive query
                    System.out.println("Archiving class with ID: " + classId);
                    insertArchivePstmt.setInt(1, classId);
                    insertArchivePstmt.executeUpdate();

                    // Delete schedules
                    System.out.println("Deleting schedules for class ID: " + classId);
                    deleteSchedulesPstmt.setInt(1, classId);
                    deleteSchedulesPstmt.executeUpdate();

                    // Delete class
                    System.out.println("Deleting class with ID: " + classId);
                    deleteClassPstmt.setInt(1, classId); // Add this line!
                    ...
                    if (rowsAffected > 0) {
                        ...
                        failedDeletes.add(className);
                    }
                } catch (SQLException e) {
                    failedDeletes.add(className);
                    System.err.println("Error archiving or deleting class " + className + ": " + e.getMessage());
                }
            }
            ...
            // Close resources
            try {
                if (insertArchivePstmt != null) {
                    insertArchivePstmt.close();
                }
                if (deleteSchedulesPstmt != null) {
                    deleteSchedulesPstmt.close();
                }
                if (deleteClassPstmt != null) {
                    deleteClassPstmt.close();
                }
                if (conn != null) {
...
// Provide feedback to the user
        if (successfulDeletes > 0) {
            ...
            message.append(successfulDeletes).append(" class(es) archived and deleted successfully");
            ...
            JOptionPane.showMessageDialog(this,
                    "No classes were archived or deleted. Please check your selection.",
                    "Operation Failed",
                    JOptionPane.WARNING_MESSAGE);
        }
